#!/bin/bash
# FILE: ~/mythosblock-rebuild/scripts/mythos-scanner-nexus.sh
# PURPOSE: Generate the Mythosblock GitHub Scanning Manifest
# MODE: Planning-only, deterministic, macOS-safe, detached-hash integrity
...
MANIFEST_FILE="${MANIFEST_DIR}/github-scanning-manifest.md"
HASH_FILE="${MANIFEST_FILE}.sha256"
TIMESTAMP="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
MANIFEST_UID="$(uuidgen | tr '[:upper:]' '[:lower:]')"

mkdir -p "${MANIFEST_DIR}"
mkdir -p "${SCRIPT_DIR}"

cat > "${MANIFEST_FILE}" <<EOF_MD
---
...
- Manifest UID: ${MANIFEST_UID}
- Generated At: ${TIMESTAMP}
- Canonical SHA-256: stored in detached file \`github-scanning-manifest.md.sha256\`
EOF_MD

shasum -a 256 "${MANIFEST_FILE}" > "${HASH_FILE}"

echo "---------------------------------------------------"
echo "Mythosblock Scanning Manifest Generated"
